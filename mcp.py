...
def send_mcp_message(message):
    ...
    Embedding model name in payload is now explicitly logged if present.
    """
    try:
        ...
        # Explicitly log embedding model if sent as part of payload
        emb_model = message.get("payload", {}).get("embedding_model_name")
        if emb_model:
            logger.info(f"Embedding model used in payload: {emb_model}")
...
def receive_mcp_message(receiver_name):
    ...
    try:
        if receiver_name in message_queues and message_queues[receiver_name]:
            ...
            emb_model = message.get("payload", {}).get("embedding_model_name")
            if emb_model:
                logger.info(f"Embedding model in received payload: {emb_model}")
            return message
        else:
...
def test_mcp():
    ...
    # Test 1: IngestionAgent → RetrievalAgent (with embedding model)
    ...
    # Test 2: RetrievalAgent → LLMResponseAgent (with embedding model)
    ...
    if msg:
        ...
        print(f"    Embedding model: {msg['payload'].get('embedding_model_name', 'N/A')}")
